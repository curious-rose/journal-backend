...
class DataLoader {
    ...
    getJournalEntries(userid, daysAgo, searchTerm, moodLimit) {
        ...
        let timeStampQuery = moment().utc().subtract(daysAgo, "days").format()
    ...
    // getGeotaggedEntries(userid, daysAgo, searchTerm, moodLimit) {
    //     //potentially, we don't actually need this second query anymore. We are just pulling all posts within
    //     //a given time period, so we could just filter entries with "lat" on the front end. 
    //     console.log("looking for geotagged entries from", daysAgo, "days ago")
    ...
    //             .orderBy("createdAt", "desc")
    //             .toString()
    //     console.log("looking for geotags: querying entries table with query:", queryString)
