...
class DataLoader {
    ...
    getJournalEntries(userid, daysAgo,searchTerm) {
        console.log("looking for all entries from",daysAgo,"days ago")
    ...
    getGeotaggedEntries(userid, daysAgo) {
        console.log("looking for geotagged entries from",daysAgo,"days ago")
        //should be in format 2017-11-05 03:39:28
        let timeStampQuery = moment().utc().subtract(daysAgo,"days").format()
        let queryString =
            knex('entries')
            .select("id","title","mood","createdAt","lat","lng","thumbnail_image_url")
